...
object StateAdminReportJob extends optional.Application with IJob with StateAdminReportHelper {
  ...
  // $COVERAGE-ON$ Enabling scoverage for other methods
   def generateExternalIdReport() (implicit sparkSession: SparkSession, fc: FrameworkContext) = {
       ...
       val activeSelfDeclaredDF = userSelfDeclaredDataDF.join(activeConsentDF, userSelfDeclaredDataDF.col("orgid") === activeConsentDF.col("consumer_id")).
           select(userSelfDeclaredDataDF.col("*"))
        val userSelfDeclaredUserInfoDataDF = activeSelfDeclaredDF.select(col("*"), col("userinfo").getItem(DECLARED_EMAIL).as(DECLARED_EMAIL), col("userinfo").getItem("declared-phone").as("declared-phone"),
       ...
       val locationDF = locationData()
        ...
        val userDecrpytedDataDF = decryptPhoneEmailInDF(userSelfDeclaredExtIdDF, "DECLARED_EMAIL", "declared-phone")
