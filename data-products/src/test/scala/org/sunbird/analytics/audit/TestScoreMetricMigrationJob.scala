...
class TestScoreMetricMigrationJob extends BaseReportSpec with MockFactory {
  ...
  it should "Should migrate the score metrics data" in {
    ...
    result.head.get(2).asInstanceOf[Seq[String]].head should be("""{"max_score":10.0,"score":10.0,"type":"score_metrics","attempt_id":"attempat-001","content_id":"do_112876961957437440110"}""")
    result.head.get(2).asInstanceOf[Seq[String]](1) should be("""{"max_score":10.0,"score":10.0,"type":"score_metrics","attempt_id":"attempat-001","content_id":"do_112876961957437440179"}""")
    ...
    result2.head.get(0).asInstanceOf[Seq[String]].head should be("""{"max_score":15.0,"score":15.0,"type":"score_metrics","attempt_id":"attempat-001","content_id":"do_11307593493010022418"}""")
    result2.head.get(0).asInstanceOf[Seq[String]](1) should be("""{"max_score":15.0,"score":10.0,"type":"score_metrics","attempt_id":"attempat-002","content_id":"do_11307593493010022418"}""")
  ...
  it should "Should migrate the score metrics data for specific batch" in {
    implicit val mockFc: FrameworkContext = mock[FrameworkContext]
    val strConfig = """{"search":{"type":"none"},"model":"org.sunbird.analytics.audit.ScoreMetricMigrationJob","modelParams":{"store":"azure","sparkCassandraConnectionHost":"localhost", "fromDate":"$(date --date yesterday '+%Y-%m-%d')","toDate":"$(date --date yesterday '+%Y-%m-%d')", "batchId": ["batch-002"]},"parallelization":8,"appName":"ScoreMetricMigrationJob"}""".stripMargin
    implicit val jobConfig: JobConfig = JSONUtils.deserialize[JobConfig](strConfig)
    implicit val sc: SparkContext = spark.sparkContext
    val res = ScoreMetricMigrationJob.migrateData(spark, List[String]("batch-002"))
    ...
    result.head.get(2).asInstanceOf[Seq[String]].head should be("""{"max_score":15.0,"score":10.0,"type":"score_metrics","attempt_id":"attempat-001","content_id":"do_11307593493010022419"}""")
  }
}